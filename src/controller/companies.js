...
const addcompany = async(req, res) =>{
  ...
  if(error){
    ...
    if(error.stack == `ValidationError: "mfo" length must be less than or equal to 10 characters long`){
      return res.status(400).json({message:"Kompaniya mfo bolimida 10 ta harfdan kopayib ketgan !"})
    }
    if(error.stack == `ValidationError: "mfo" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya mfo bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "accaunt" length must be less than or equal to 30 characters long`){
      return res.status(400).json({message:"Kompaniya akkaunt bolimida 30 ta harfdan oshib ketgan !"})
    }
    if(error.stack == `ValidationError: "accaunt" must be a valid email`){
      return res.status(400).json({message:"Kompaniya akkaunt bolimid email notogri shakld kiritilgan !"})
    }
    if(error.stack == `ValidationError: "accaunt" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya akkaunt bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "bank" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya bank bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "type" must be one of [1, 2]`){
      return res.status(400).json({message:"Kompaniya type bolimiga faqat 1 va 2 chi turini tanlash kerak !"})
    }
    if(error.stack == `ValidationError: "type" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya type bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "director" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya director bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "adress" is not allowed to be empty`){
      return res.status(400).json({message:"Kompaniya address bolimi toldirilmagan !"})
    }
    if(error.stack == `ValidationError: "phone" must be a number`){
      return res.status(400).json({message:"Kompaniya telefon raqami bolimi toldirilmagan !"})
    };
    if(error.stack == `ValidationError: "phone" must be greater than or equal to 330000001`){
      return res.status(400).json({message:"Kompaniya telefon raqami bolimiga notogri raqam kirgizilmoqda"})
    }
    if(error.stack == `ValidationError: "phone" must be less than or equal to 999999999`){
      return res.status(400).json({message:"Kompaniya telefon raqami bolimiga kop raqam kirgizilmoqda kirgizilmoqda"})
    } 
  }
